...
#if (CONFIG_HARDWARE_CONNECTION == DJI_USE_ONLY_UART)
    USER_LOG_INFO("Hardware connection mode: UART only, device=%s, baud=%s.",
                  LINUX_UART_DEV1, USER_BAUD_RATE);
#endif
}
...
void Application::DjiUser_ApplicationStart()
{
    ...
    USER_LOG_INFO("Aircraft base info: series=%d, type=%d, mountType=%d, mount=%d, adapter=%d.",
                  static_cast<int>(aircraftInfoBaseInfo.aircraftSeries),
                  static_cast<int>(aircraftInfoBaseInfo.aircraftType),
                  static_cast<int>(aircraftInfoBaseInfo.mountPositionType),
                  static_cast<int>(aircraftInfoBaseInfo.mountPosition),
                  static_cast<int>(aircraftInfoBaseInfo.djiAdapterType));

    if (aircraftInfoBaseInfo.aircraftSeries == DJI_AIRCRAFT_SERIES_FC30 ||
        aircraftInfoBaseInfo.aircraftType == DJI_AIRCRAFT_TYPE_FC30) {
        USER_LOG_INFO("FC30 aircraft detected.");
    }
    ...
    if (aircraftInfoBaseInfo.mountPosition != DJI_MOUNT_POSITION_EXTENSION_PORT &&
        ...
        aircraftInfoBaseInfo.djiAdapterType != DJI_SDK_ADAPTER_TYPE_SKYPORT_V2) {
        USER_LOG_WARN("Mount/adapter is outside the official 3.14.1 extension/E-Port allowlist; "
                      "continuing to preserve Payload Port compatibility.");
    }
